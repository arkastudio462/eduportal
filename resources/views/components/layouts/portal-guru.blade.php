...
<body class="bg-background text-on-surface">
    <x-loading-screen />
    <x-portal-guru-sidebar />
    <main class="md:ml-64 min-h-screen">
        <x-portal-topbar :with-logo="true" title="EduPortal" />
        <div class="p-margin-desktop max-md:p-margin-mobile">
            {{ $slot }}
        </div>
    </main>
    ...
    <nav class="md:hidden fixed bottom-0 left-0 right-0 bg-surface border-t border-outline-variant px-6 py-3 flex justify-between items-center z-50">
        <a class="flex flex-col items-center gap-1 {{ request()->routeIs('portal-guru.dashboard') ? 'text-primary' : 'text-on-surface-variant' }}" href="{{ route('portal-guru.dashboard') }}">
            <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' {{ request()->routeIs('portal-guru.dashboard') ? 1 : 0 }};">dashboard</span>
            <span class="text-[10px] font-bold">Dash</span>
        </a>
        <a class="flex flex-col items-center gap-1 {{ request()->routeIs('portal-guru.jadwal') ? 'text-primary' : 'text-on-surface-variant' }}" href="{{ route('portal-guru.jadwal') }}">
            <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' {{ request()->routeIs('portal-guru.jadwal') ? 1 : 0 }};">calendar_month</span>
            <span class="text-[10px] font-medium">Jadwal</span>
        </a>
        <a class="flex flex-col items-center gap-1 {{ request()->routeIs('portal-guru.nilai') ? 'text-primary' : 'text-on-surface-variant' }}" href="{{ route('portal-guru.nilai') }}">
            <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' {{ request()->routeIs('portal-guru.nilai') ? 1 : 0 }};">grade</span>
            <span class="text-[10px] font-medium">Nilai</span>
        </a>
        <a class="flex flex-col items-center gap-1 {{ request()->routeIs('portal-guru.tugas') ? 'text-primary' : 'text-on-surface-variant' }}" href="{{ route('portal-guru.tugas') }}">
            <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' {{ request()->routeIs('portal-guru.tugas') ? 1 : 0 }};">assignment</span>
            <span class="text-[10px] font-medium">Tugas</span>
        </a>
        <a class="flex flex-col items-center gap-1 {{ request()->routeIs('portal-guru.profil') ? 'text-primary' : 'text-on-surface-variant' }}" href="{{ route('portal-guru.profil') }}">
            <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' {{ request()->routeIs('portal-guru.profil') ? 1 : 0 }};">account_circle</span>
