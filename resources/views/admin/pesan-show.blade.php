<x-layouts.admin title="{{ $conversation->subjek ?? 'Pesan' }}">
<x-slot:styles>
...
<div x-data="{
    ...
    saveEdit(id) {
        ...
        if (!body) return;
        let baseUrl = '{{ route('admin.pesan.update', [$conversation, '__MID__']) }}'.replace('__MID__', id);
        fetch(baseUrl, {
            method: 'PUT',
    ...
    deleteSelected() {
        ...
        fetch('{{ route('admin.pesan.bulk-destroy', $conversation) }}', {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content, 'Accept': 'application/json' },
    ...
    send() {
        ...
        fetch('{{ route('admin.pesan.reply', $conversation) }}', {
            method: 'POST',
...
">
    ...
    <div class="flex items-center gap-4 mb-8">
        <a href="{{ route('admin.pesan') }}" class="inline-flex items-center gap-1 text-outline hover:text-primary transition-colors">
