<x-layouts.portal-guru title="{{ $conversation->subjek ?? 'Pesan' }}">
<x-slot:styles>
...
<div x-data="{
    ...
    saveEdit(id) {
        ...
        if (!body) return;
        let baseUrl = '{{ route('portal-guru.pesan.update', [$conversation, '__MID__']) }}'.replace('__MID__', id);
        fetch(baseUrl, {
            method: 'PUT',
    ...
    deleteSelected() {
        ...
        fetch('{{ route('portal-guru.pesan.bulk-destroy', $conversation) }}', {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content, 'Accept': 'application/json' },
    ...
    send() {
        ...
        fetch('{{ route('portal-guru.pesan.reply', $conversation) }}', {
            method: 'POST',
...
">
    ...
    <div class="flex items-center gap-4 mb-8">
        <a href="{{ route('portal-guru.pesan') }}" class="inline-flex items-center gap-1 text-outline hover:text-primary transition-colors">
