<x-layouts.portal-guru title="Bank Soal - Portal Guru">
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
    <div>
        <h2 class="font-headline-lg text-headline-lg text-primary">Bank Soal</h2>
        <p class="text-on-surface-variant font-body-md">Kelola soal untuk {{ $mapel?->nama ?? 'mata pelajaran Anda' }}</p>
    </div>
    @if($mapel)
    <div class="flex gap-3">
        <a href="{{ route('portal-guru.bank-soal.download-template') }}"
           class="px-4 py-2.5 border border-outline-variant rounded-lg flex items-center gap-2 font-label-md text-outline hover:bg-surface-container transition-all">
        ...
        <form method="POST" action="{{ route('portal-guru.bank-soal.import') }}" enctype="multipart/form-data" class="hidden">
            @csrf
            <input type="file" name="file" id="import-file" accept=".json,.txt" onchange="this.form.submit()">
        </form>
        <a href="{{ route('portal-guru.bank-soal') }}?tambah=1"
...
<div class="bg-white rounded-xl border border-outline-variant card-shadow overflow-hidden" x-data="soalApp()">
    <div class="p-4 md:p-6 border-b border-outline-variant">
        <form method="GET" action="{{ route('portal-guru.bank-soal') }}" class="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <div class="relative">
            ...
            <button type="submit" class="px-4 py-2 bg-secondary-container text-on-secondary-container rounded-lg font-label-md text-sm hover:bg-secondary-container/70 transition-all">Terapkan Filter</button>
        </form>
    </div>
    ...
    <div class="divide-y divide-outline-variant">
        ...
        <div class="p-4 md:p-6 hover:bg-surface-container transition-all">
            <div class="flex items-start justify-between gap-4">
                ...
                <div class="flex items-center gap-1 shrink-0">
                    ...
                    <form method="POST" action="{{ route('portal-guru.bank-soal.destroy', $soal->id) }}" id="delete-form-{{ $soal->id }}" class="hidden">
                        @csrf
                        @method('DELETE')
                    </form>
                </div>
        ...
        @empty
        <div class="p-6 text-on-surface-variant text-center">Tidak ada soal. Tambah soal baru atau import dari file JSON.</div>
        @endforelse
    </div>
    ...
    <div x-show="openModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-effect="if (openModal) { $nextTick(() => { document.body.style.overflow = 'hidden'; }) } else { document.body.style.overflow = '' }">
        ...
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto z-10 p-6">
            <form method="POST" enctype="multipart/form-data" :action="editId ? '{{ route('portal-guru.bank-soal.update', '__ID__') }}'.replace('__ID__', editId) : '{{ route('portal-guru.bank-soal.store') }}'">
                @csrf
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-6 py-2.5 bg-primary text-on-primary rounded-lg font-label-md hover:bg-primary/90 transition-all active:scale-95" x-text="editId ? 'Simpan Perubahan' : 'Simpan'"></button>
                </div>
            </form>
        </div>
    </div>
...
<div class="bg-white rounded-xl border border-outline-variant card-shadow p-12 text-center">
    <span class="material-symbols-outlined text-6xl text-outline mb-4">library_books</span>
    <h3 class="font-headline-sm text-headline-sm text-on-surface mb-2">Belum Ada Mata Pelajaran</h3>
    <p class="text-on-surface-variant">Anda belum memiliki mata pelajaran yang ditugaskan. Hubungi admin untuk informasi lebih lanjut.</p>
</div>
@endif
...
function soalApp() {
    return {
        ...
        resetForm() {
            this.editId = null;
            this.form = { mapel_id: '{{ $mapel?->id ?? '' }}', tipe: '', kesulitan: '', konten: '', gambar: '', gambarPreview: null, opsi: [{label: 'A', value: ''}, {label: 'B', value: ''}], jawaban: '' };
        },
        editSoal(id) {
            fetch('{{ route('portal-guru.bank-soal.show', '__ID__') }}'.replace('__ID__', id))
                .then(r => r.json())
                .then(data => {
                    this.editId = data.id;
