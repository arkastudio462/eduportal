<x-layouts.portal-guru title="Nilai - Portal Guru">
<x-slot:styles>
<style>[x-cloak] { display: none !important; }</style>
</x-slot:styles>
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
    <div>
        <h2 class="font-headline-lg text-headline-lg text-primary">Input Nilai</h2>
        <p class="text-on-surface-variant font-body-md">Input nilai siswa per kelas dan mata pelajaran</p>
    </div>
</div>
...
<form method="GET" class="bg-white p-6 rounded-xl border border-outline-variant card-shadow mb-6">
    <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <div>
            <label class="font-label-sm text-label-sm text-outline mb-1 block">Mata Pelajaran</label>
            <select name="mapel_id" class="w-full px-3 py-2 border border-outline-variant rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none bg-white" onchange="this.form.submit()">
                @foreach($mapelList as $mapel)
                <option value="{{ $mapel->id }}" {{ ($selectedMapel->id ?? null) == $mapel->id ? 'selected' : '' }}>{{ $mapel->nama }}</option>
                @endforeach
            </select>
        </div>
        <div>
            <label class="font-label-sm text-label-sm text-outline mb-1 block">Jenis</label>
            <select name="jenis" class="w-full px-3 py-2 border border-outline-variant rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none bg-white" onchange="this.form.submit()">
                <option value="uh" {{ request('jenis') == 'uh' ? 'selected' : '' }}>Ulangan Harian</option>
        ...
        <div>
            ...
            <select name="semester" class="w-full px-3 py-2 border border-outline-variant rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none bg-white" onchange="this.form.submit()">
                <option value="Ganjil" {{ request('semester') == 'Ganjil' ? 'selected' : '' }}>Ganjil {{ $tahunAjar }}</option>
                <option value="Genap" {{ request('semester') == 'Genap' ? 'selected' : '' }}>Genap {{ $tahunAjar }}</option>
            </select>
        </div>
    </div>
</form>
...
<form method="POST" action="{{ route('portal-guru.nilai.store') }}">
    @csrf
    <input type="hidden" name="kelas_id" value="{{ $selectedKelas->id }}">
    <input type="hidden" name="mapel_id" value="{{ $selectedMapel->id }}">
    <input type="hidden" name="jenis" value="{{ request('jenis', 'uh') }}">
    <input type="hidden" name="semester" value="{{ request('semester', $semester) }}">
            ...
            <table class="w-full text-left">
                <thead class="bg-surface-container-low">
                    <tr>
                        ...
                        <th class="px-6 py-4 font-label-md text-on-surface-variant">Nilai (0-100)</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-outline-variant">
                    @foreach($siswaList as $index => $siswa)
                    @php $nilaiSiswa = $dataNilai->get($siswa->id); @endphp
                    <tr class="hover:bg-surface-container transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <input type="number" name="nilai[{{ $siswa->id }}]" value="{{ $nilaiSiswa->skor ?? '' }}"
                                class="w-24 px-3 py-2 border border-outline-variant rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none"
...
<div class="bg-white rounded-xl border border-outline-variant card-shadow p-8 text-center">
    <span class="material-symbols-outlined text-5xl text-outline mb-4">group_off</span>
    <h3 class="font-headline-sm text-headline-sm text-on-surface mb-2">Tidak Ada Siswa</h3>
...
<div class="bg-white rounded-xl border border-outline-variant card-shadow p-8 text-center">
    ...
    <h3 class="font-headline-sm text-headline-sm text-on-surface mb-2">Pilih Kelas & Mapel</h3>
    <p class="text-on-surface-variant">Silakan pilih kelas dan mata pelajaran untuk mulai input nilai.</p>
</div>
@endif
