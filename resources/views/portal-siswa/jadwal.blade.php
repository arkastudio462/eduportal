<x-layouts.portal-siswa title="Jadwal Pelajaran - Portal Siswa">
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
    <div>
        <h2 class="font-headline-lg text-headline-lg text-primary">Jadwal Pelajaran</h2>
        <p class="text-on-surface-variant font-body-md">Lihat jadwal pelajaran mingguan Anda</p>
    </div>
    <a href="{{ route('portal-siswa.jadwal.print') }}" target="_blank" class="px-4 py-2.5 border border-outline-variant rounded-lg flex items-center gap-2 font-label-md text-outline hover:bg-surface-container transition-all">
...
<div class="bg-white rounded-xl border border-outline-variant card-shadow p-8 text-center">
    ...
    <p class="text-on-surface-variant">Jadwal pelajaran belum tersedia untuk kelas Anda.</p>
</div>
@else
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-gutter">
    ...
    <div class="bg-white rounded-xl border border-outline-variant card-shadow overflow-hidden">
        <div class="px-5 py-4 bg-primary-fixed border-b border-outline-variant flex items-center gap-3">
            <span class="w-3 h-3 rounded-full {{ $jadwalHari->isNotEmpty() ? 'bg-green-500' : 'bg-outline/30' }}"></span>
            ...
            <div class="p-4 bg-surface-container-low rounded-xl border border-outline-variant">
                <div class="flex items-start justify-between gap-2 mb-2">
                    <div>
                        ...
                        <span class="text-xs text-outline ml-2">{{ $j->guru->user->name ?? '-' }}</span>
                    </div>
                    <span class="text-xs text-outline font-mono shrink-0">{{ $j->jam_mulai ? \Carbon\Carbon::parse($j->jam_mulai)->format('H:i') : '-' }} - {{ $j->jam_selesai ? \Carbon\Carbon::parse($j->jam_selesai)->format('H:i') : '-' }}</span>
                </div>
                @if($j->ruang)
